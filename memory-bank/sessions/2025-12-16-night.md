...
## Task Updates
- Narrowed T24 scope to sql.js migration only
- Created T25 for standalone Node package (browser-first)
- Added browser database update workflow scope (import all markdown + DB -> markdown regeneration) to T21

## Import Flow Updates
- Added browser import flows for tasks/sessions/session_cache and synced them into `mb-cli/templates/memory-bank/database/` via `mb-cli/src/sync-database-template.js`
- Verified fresh init requires viewer files (`--setup-viewer` or full init) to generate `memory-bank/database/server.js`
