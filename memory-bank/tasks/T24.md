...
**Description**: Replace `better-sqlite3` (native addon) with `sql.js` (pure WASM) across the codebase to enable predictable installation without Node/V8 ABI friction. This migration affects both the standalone server package and all CLI template parser/query scripts.

**Status**: 🔄 In Progress
**Priority**: HIGH
**Started**: 2025-12-16
**Last Active**: 2026-02-12 16:44:15 IST
**Dependencies**: T19, T21, T13

## Context

Previous work completed:
- ✅ Established a sql.js adapter pattern compatible with existing server/parsers
- ✅ Identified all better-sqlite3 touchpoints that must migrate
...
Progress made (2025-12-16 night):
- Canonicalized the standalone database package under `mb-cli/src/server-package/` and added a sync script to regenerate `mb-cli/templates/memory-bank/database/`.
- Fixed sql.js adapter persistence correctness (avoid saving unless writes occurred) and improved better-sqlite3 compatibility (`lastInsertRowid`).
- Fixed server/init scripts to use async transactions correctly and ensured default DB bootstrap behavior.
- Fixed `mb init --interactive` viewer copying flag (`setupViewer`) and added `pnpm-workspace.yaml` to prevent monorepo install hijack.

Progress made (2026-02-12 afternoon):
- Fixed parser runtime/syntax issues in `memory-bank/database/parse-edits.js`, `parse-tasks.js`, `parse-sessions.js`, and `parse-session-cache.js` by removing undefined `db` usage and correcting async transaction/schema patterns.
- Validated parser execution and syntax (`node --check`) after fixes.
- Propagated parser fixes to canonical `mb-cli/src/server-package/` and regenerated `mb-cli/templates/memory-bank/database/` so newly initialized projects inherit corrected scripts.

## Completion Criteria

### Phase 1: Server Package Migration
- [ ] Ensure `mb-cli/src/server-package/lib/sqlite.js` adapter is complete and compatible
- [ ] Ensure `mb-cli/src/server-package/package.json` has no native sqlite deps
- [ ] Ensure `mb-cli/src/server-package/server.js` uses sql.js adapter everywhere
...
- `mb-cli/src/server-package/package.json`
...
## Success Metrics
- ✅ `npm install` succeeds without compilation
